...
let mode=(!client)?'local':'supabase',
    products=[],
    reviews=[],
    settings={},
    user=null;
...
function show(v){
  $$('[data-admin-view]').forEach(b=>
    b.classList.toggle('active',b.dataset.adminView===v)
  );
...
function localLoad(){
  try{
    products=
      JSON.parse(localStorage.getItem('dlsn-admin-products')||'null') ||
      JSON.parse(JSON.stringify(base));
  }catch{
    products=JSON.parse(JSON.stringify(base));
  }

  try{
    reviews=JSON.parse(
      localStorage.getItem('dlsn-local-reviews')||'[]'
    );
  }catch{
    reviews=[];
  }
  ...
  try{
    settings=JSON.parse(
      localStorage.getItem('dlsn-site-settings')||'{}'
    );
  }catch{
    settings={};
  }
...
function enter(){
  ...
  const account=$('#adminAccountEmail');
...
async function saveProduct(e){
  ...
  if(mode==='local'){
    ...
    const localPayload={
      ...
      url:
        `produto.html?slug=${payload.slug}`
    ...
    reviews.map(r=>`
      ...
      <tr>
        ...
        <td>
          ...
          <button
            class="admin-btn"
            data-review-status="${r.id}:approved">
            Publicar
          </button>

          <button
            class="admin-btn"
            data-review-status="${r.id}:rejected">
            Ocultar
          </button>
...
function renderSettings(){
  ...
  if($('#settingAnnouncement'))
    $('#settingAnnouncement').value=
      s.announcement||'';

  if($('#settingHeroBanner'))
    $('#settingHeroBanner').value=
      s.hero_banner||'';
  ...
  if($('#settingWhatsapp'))
    $('#settingWhatsapp').value=
      s.whatsapp||'5567996773494';

  if($('#settingStoreDescription'))
    $('#settingStoreDescription').value=
      s.store_description||'';

  if($('#settingFooterNote'))
    $('#settingFooterNote').value=
      s.footer_note||'';
  ...
  if($('#settingLogoSpin'))
    $('#settingLogoSpin').value=
      s.logo_spin_seconds||8;
